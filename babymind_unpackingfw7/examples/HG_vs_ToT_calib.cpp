...
Double_t bestfit(Double_t *x, Double_t *par) {
    ...
    //return par[0] + (par[1])/(par[2] - TMath::Exp(-par[3]*(x[0]-par[4])));
   }
...
int main ()
{
    ...
    rootFileOutput+="_HG_ToT_calib.root";
  ...
  TH2F *HG_vs_ToT[NumberOfFEB][96];
  Double_t fitPar[6];
  TF1 *f = new TF1("f",bestfit,4,20,5);

  Double_t fitLin1[2];
  TF1 *fline1 = new TF1("fline1",lineFit,0,6,2);

  Double_t fitLin2[2];
  TF1 *fline2 = new TF1("fline2",lineFit,15,23,2);
  ...
  for (int ih = 0; ih<FEBnumbers.size();ih++) {
        ...
        string fouttxt = "HG_ToT_calib/";
        fouttxt+=sFEB.c_str();
        fouttxt+="_HG_ToT_calib.txt";
        ofstream foutDat(fouttxt.c_str());
        TDirectory *FEB_HG_ToT = wfile.mkdir(sFEB.c_str());
        ...
        for (Int_t ich=0; ich < 96;ich++){
            ...
            HG_vs_ToT[FEBnumbers.at(ih)][ich]=new TH2F(sCh.c_str(),sCh.c_str(),  51, 0, 50, 4096, 0, 4095);
        }
        ...
        for (Int_t enu = 0; enu<nentries[FEBnumbers.at(ih)]; enu++){
            FEBtree[FEBnumbers.at(ih)]->GetEntry(enu);
                for (Int_t i = 0; i<FEB[FEBnumbers.at(ih)].hitTimeDif->size(); i++){
                    if (FEB[FEBnumbers.at(ih)].hitAmplMatched->at(i)>0 && FEB[FEBnumbers.at(ih)].hitTimeDif->at(i)>0 && FEB[FEBnumbers.at(ih)].hitAmplMatched->at(i) < 3500) {
                        HG_vs_ToT[FEBnumbers.at(ih)][(int)FEB[FEBnumbers.at(ih)].hitsChannel->at(i)]-> Fill(FEB[FEBnumbers.at(ih)].hitTimeDif->at(i),FEB[FEBnumbers.at(ih)].hitAmplMatched->at(i) , 1);
                }
            }
        }
        ...
        FEB_HG_ToT->cd();
        for (Int_t ich=0; ich < 96;ich++){
            if (HG_vs_ToT[FEBnumbers.at(ih)][ich]->GetEntries()>1) {
                
                
               
                TProfile *prof = HG_vs_ToT[FEBnumbers.at(ih)][ich]->ProfileX();
            ...
            delete HG_vs_ToT[FEBnumbers.at(ih)][ich];
        ...
        FEB[FEBnumbers.at(ih)].hitAmplMatched->clear();
