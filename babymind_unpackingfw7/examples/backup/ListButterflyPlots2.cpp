...
int main (int argc, char **argv)
{
  ...
  TH1D *proximity_norm = new TH1D("prox_norm","",32,0,32);
  ...
  foutDir = GetDir(vFileNames.at(0)) + "/Baseline_noise_2/";
  ...
  for (vector<string>::iterator itFileName=vFileNames.begin(); itFileName != vFileNames.end(); itFileName++){
    ...
    cout<< "Number of Spills "<< FEBtree->GetEntries()<<"\n"<<endl;



    vector<Int_t> laserHits;
    ...
    for (Int_t Spill = 0; Spill<FEBtree->GetEntries(); Spill++){
      ...
      //loop over spill hits to find the laser hits
      for (Int_t hit1 = 0; hit1<FEB.SpillTag->size(); hit1++){
        //check if hit1 is on Channel 64 (laser)
        if (FEB.hitsChannel->at(hit1)!=64) continue;

        //Check if hit1 has a physical amplitude expected for a laser pulse
        if (FEB.hitTimeDif->at(hit1)<20 || FEB.hitTimeDif->at(hit1)>50) continue;

        laserHits.push_back(hit1);
        count_bl=0;

        //only consider spills with max 11 laser hits
        if (laserHits.size()<12){
          //loop over laser hits, and check neighboring hits (in time) for each
          for(Int_t i=0; i<laserHits.size(); i++){
            hit1 = laserHits.at(i);

            for(int hit2=0; hit2<FEB.SpillTag->size(); hit2++){
              //make sure hit2 != hit1 (+ does not appear in the laserHits vector)
              if (find(laserHits.begin(), laserHits.end(), hit2) != laserHits.end()) continue;
              //if (FEB.hitsChannel->at(hit2)==64) continue;

              double TimeBtHits = FEB.hitTimefromSpill->at(hit2)-FEB.hitTimefromSpill->at(hit1);

              if (TimeBtHits>0 && TimeBtHits<100){
                count_bl ++;
