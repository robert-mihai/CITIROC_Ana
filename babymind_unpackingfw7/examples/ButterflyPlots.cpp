...
int main (int argc, char **argv)
{
  ...
  string foutDir = GetDir(sFileName) + "/Baseline_noise_2/";
  ...
  TH2D *bl_Amplitude = new TH2D("NB_Ch","",100,0,1000,100,0,100);
  ...
  for (Int_t Spill = 0; Spill<FEBtree->GetEntries(); Spill++){
    ...
    for (Int_t hit1 = 0; hit1<FEB.SpillTag->size(); hit1++){
      ...
      for (Int_t hit_check = 0; hit_check<FEB.SpillTag->size(); hit_check++){
        ...
        if (abs(FEB.hitTimefromSpill->at(hit_check)-FEB.hitTimefromSpill->at(hit1))<100 && FEB.hitTimeDif->at(hit_check)>FEB.hitTimeDif->at(hit1))
            notmaximum = true;
      }
